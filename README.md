...
- **Mode d’emploi rapide** : dans l’éditeur, ajoutez le bloc **Lightbox – Aperçu** depuis l’insertion rapide (`/lightbox`), puis ajustez les paramètres dans la colonne de droite (lecture automatique, boucle, opacité du fond, etc.). Les modifications sont reflétées instantanément dans le rendu miniature, ce qui permet de prévisualiser votre configuration avant de l’appliquer à de vraies galeries.
